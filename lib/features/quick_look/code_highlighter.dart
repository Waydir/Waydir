...
const _cFamilyKw = {
  ...
  'continue', 'return', 'goto', 'struct', 'enum', 'union', 'typedef', 'sizeof',
  'static', 'const', 'volatile', 'extern', 'inline', 'void', 'int', 'char',
  'float', 'double', 'long', 'short', 'unsigned', 'signed', 'bool', 'class',
  'public', 'private', 'protected', 'virtual', 'override', 'namespace',
  'template', 'typename', 'using', 'new', 'delete', 'this', 'true', 'false',
  'nullptr', 'auto', 'try', 'catch', 'throw',
};
...
const _dartTypes = {
  ...
  'Stream', 'Object', 'Iterable', 'Widget', 'BuildContext', 'override',
};
...
const _jsKw = {
  ...
  'typeof', 'undefined', 'var', 'void', 'while', 'yield', 'readonly', 'keyof',
  'namespace', 'declare',
};
...
const _goKw = {
  ...
  'var', 'true', 'false', 'nil', 'string', 'int', 'bool', 'error',
};
...
final Map<String, CodeLanguage> _byExt = {
  ...
  for (final e in [
    'c', 'h', 'cpp', 'cc', 'cxx', 'hpp', 'java', 'kt', 'cs', 'go', 'swift',
    'scala',
  ])
    e: CodeLanguage(
  ...
  for (final e in [
    'py', 'pyw', 'rb', 'sh', 'bash', 'zsh', 'fish', 'toml', 'ini', 'conf',
    'cfg',
  ])
    e: const CodeLanguage(keywords: _pyKw, lineComment: '#'),
  for (final e in ['yaml', 'yml'])
    e: const CodeLanguage(lineComment: '#'),
  for (final e in ['json', 'jsonc'])
    e: const CodeLanguage(lineComment: '//'),
  'sql': const CodeLanguage(
    keywords: {
      ...
      'and', 'or', 'not', 'null', 'as', 'distinct', 'union', 'index',
    },
    lineComment: '--',
    blockComment: ['/*', '*/'],
  ),
...
List<TextSpan> highlightCode(String text, CodeLanguage lang, TextStyle base) {
  ...
  for (final m in re.allMatches(text)) {
    ...
    final tok = m[0]!;
    TextStyle st;
    if (m.namedGroup('bc') != null ||
        m.namedGroup('lc') != null ||
        m.namedGroup('lc2') != null) {
      st = commentStyle;
    } else if (m.namedGroup('num') != null) {
      st = numberStyle;
    } else if (m.namedGroup('id') != null) {
